        ...
        <div className='flex flex-col pt-10 border-t-2 border-gray-100 border-opacity-20'>
            <div className="flex flex-col md:flex-row justify-between items-center md:items-start">
                <div className="flex items-center">
                    ...
                    <div className="flex flex-col">
                        <h1 className="font-bold text-4xl mt-1">MidUI</h1>
                        <span className="self-end text-sm">❤️ Open source</span>
                    </div>
                </div>

                <div className="flex text-[#b2b2b2]  mt-10 md:mt-0 space-x-8 items-start">
                    {
                        information.map((v,i)=>{
                            return (
                                <div key={i} className="flex flex-col space-y-2 items-start">
                                    {
                                        v.map((k, _i) => <Link className="transition ease-in-out delay-100 hover:-translate-x-1 hover:text-[#0D92FC]" key={_i} href={k.href}>{k.name}</Link>)
                                    }
                                </div>
                            )
                        })
              ...
              <div className="w-full flex justify-center mt-10">
                  <div className="flex text-sm items-center select-none space-x-2">
